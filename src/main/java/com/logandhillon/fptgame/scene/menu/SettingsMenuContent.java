...
public class SettingsMenuContent implements MenuContent {
     ...
     */
    private final HashMap<KeyBind, MenuButton> keyBindButtons = new HashMap<>();
    /**
     * List of all key binds that are in use; maps {@link KeyBind} to key code string representation.
     */
    private final HashMap<KeyBind, String>     usedKeybinds   = new HashMap<>();
    ...
    public SettingsMenuContent(MenuHandler menu) {
        ...
        usedKeybinds.put(KeyBind.LEFT, config.getKeyMoveLeft());
        usedKeybinds.put(KeyBind.RIGHT, config.getKeyMoveRight());
        usedKeybinds.put(KeyBind.JUMP, config.getKeyMoveJump());
        usedKeybinds.put(KeyBind.INTERACT, config.getKeyMoveInteract());
        ...
        keyBindButtons.put(
                KeyBind.LEFT,
                new MenuButton(
                        config.getKeyMoveLeft(), 259, 457, 100, 40,
                        () -> currentKeyBind = KeyBind.LEFT));
        keyBindButtons.put(
                KeyBind.RIGHT,
                new MenuButton(
                        config.getKeyMoveRight(), 259, 513, 100, 40,
                        () -> currentKeyBind = KeyBind.RIGHT));
        keyBindButtons.put(
                KeyBind.JUMP,
                new MenuButton(
                        config.getKeyMoveJump(), 259, 569, 100, 40,
                        () -> currentKeyBind = KeyBind.JUMP));
        keyBindButtons.put(
                KeyBind.INTERACT,
                new MenuButton(
                        config.getKeyMoveInteract(), 259, 625, 100, 40,
        ...
        entities = new Entity[]{
                ...
                // control buttons
                keyBindButtons.get(KeyBind.LEFT),
                keyBindButtons.get(KeyBind.RIGHT),
                keyBindButtons.get(KeyBind.JUMP),
                keyBindButtons.get(KeyBind.INTERACT),
    ...
    private void onKeyPressed(KeyEvent e) {
        ...
        if (usedKeybinds.containsValue(code)) {
        ...
        usedKeybinds.put(currentKeyBind, code); // update map to check duplicates against
        keyBindButtons.get(currentKeyBind).setText(code); // update btn text
        currentKeyBind = null;
        updateKeyBindStatus = "";
    }
}
